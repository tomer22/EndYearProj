public class Exercise implements Comparable<Exercise> {
    ...
    public int compareTo(Exercise other) {
        // This compare to is a bit different than a normal compare to since it has 4 categories that are special to this class. A negative number implies that the exercises are completely different and do not go together in a PPL split or in the same workout. A 0 implies they're the same exercise. A 1 implies they are the same movement type and target muscle just a different machine / exercise (example: cable row and lat pull down, both are pull and target back). And a 2 implies that the movement type is the same but it does not target the same muslce (example: lat pull down and bicep curls).
